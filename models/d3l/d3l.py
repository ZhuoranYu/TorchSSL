...
class D3L:
    ...
    def train(self, args, logger=None):
        ...
        wnet = WNet(10, 100, 1).to(args.gpu)
        optimizer_wnet = torch.optim.Adam(wnet.params(), lr=6e-5)
        ssl_obj = MSE_Loss()
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s, y_ulb) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                ...
                coef = 10.0 * math.exp(-5 * (1 - min(self.it / args.num_train_iter, 1)) ** 2)

                with torch.no_grad():
                    logits = self.model(inputs)

                    logits_x_ulb_w, logits_x_ulb_s = logits[num_lb:].chunk(2)
                    cost_w = ssl_obj(logits_x_ulb_s, logits_x_ulb_w.detach())

                weight = wnet(logits_x_ulb_w.softmax(1))
                norm = torch.sum(weight)

                if norm != 0:
                    loss_weight = coef * (torch.sum(cost_w.detach() * weight) / norm)
                else:
                    loss_weight = coef * (torch.sum(cost_w.detach() * weight) )
                ...
                if norm != 0:
                    total_loss = sup_loss + coef * (torch.sum(cost_w * weight.detach()) / norm )
                else:
                    total_loss = sup_loss + coef * (torch.sum(cost_w * weight.detach()) )
