...
from .v2_utils import consistency_loss, Get_Scalar
...
class InPLV2:
    ...
    def train(self, args, logger=None):
        ...
        # initialize feature centroids
        f_dim = self.model.fc.in_features
        centroids = torch.zeros(self.num_classes, f_dim).cuda(args.gpu)
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                logits, feats = self.model(inputs, ood_test=True)
                logits_x_lb = logits[:num_lb]
                logits_x_ulb_w, logits_x_ulb_s = logits[num_lb:].chunk(2)
                sup_loss = ce_loss(logits_x_lb, y_lb, reduction='mean')

                feats_lb = feats[:num_lb]
                feats_ulb_w, feats_ulb_s = feats[num_lb:].chunk(2)
                iter_classes = torch.unique(y_lb)

                for c in iter_classes:
                    c_feat = feats_lb[y_lb == c]
                    c_feat = torch.mean(c_feat, dim=0, keepdim=True)
                    c_feat = F.normalize(c_feat.detach())

                    # update centroids
                    centroids[c] = 0.999 * centroids[c] + (1 - 0.999) * c_feat
                ...
                w_warmup = float(np.clip(self.it / (self.warmup * args.num_train_iter), 0.0, 1.0))

                total_loss = sup_loss + self.lambda_u * unsup_loss * w_warmup
