        ...
        for (_, x_lb, y_lb), (_, x_ulb_w, x_ulb_s1, x_ulb_s2, x_ulb_s1_rot, rot_v) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                with torch.no_grad():
                    ...
                    logits_x_ulb_w = self.model.classify(self.model(x_ulb_w))
                ...
                logits = [self.model.classify(self.model(mixed_x[0]))]

                self.bn_controller.freeze_bn(self.model)
                for ipt in mixed_x[1:]:
                    logits.append(self.model.classify(self.model(ipt)))

                u1_logits = self.model.classify(self.model(x_ulb_s1))
                logits_rot = self.model.classify_rot(self.model(x_ulb_s1_rot))
                ...
                abc_loss_u1 = F.cross_entropy(logits_x_ulb_s1_cb, max_indices, reduction='none') * u_mask * select_mask
                abc_loss_u1 = abc_loss_u1.mean()

                abc_loss_u2 = F.cross_entropy(logits_x_ulb_s2_cb, max_indices, reduction='none') * u_mask * select_mask
                abc_loss_u2 = abc_loss_u2.mean()

                abc_loss = abc_loss_x + abc_loss_u1 + abc_loss_u2

                total_loss = sup_loss + args.w_rot * rot_loss + w_kl * u1_loss + w_match * unsup_loss + abc_loss
